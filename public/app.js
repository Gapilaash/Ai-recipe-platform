...
// Phone camera photos are often 3-10MB, which is bigger than the hosting
// platform's request-size limit and gets rejected before it even reaches
// our backend (showing up as a generic "Request failed"). Shrink + re-encode
// on-device first so uploads stay small and fast regardless of the original.
function compressImage(file, { maxDimension = 1600, quality = 0.82 } = {}) {
  return new Promise((resolve) => {
    ...
    img.onerror = () => {
      URL.revokeObjectURL(objectUrl);
      resolve(file); // fall back to the original file if we can't decode it
    };
    img.src = objectUrl;
  });
}
...
async function handlePhotoChosen(input) {
  ...
  detectBtn.textContent = "Preparing photo…";
...
photoInputCamera.addEventListener("change", () => handlePhotoChosen(photoInputCamera));
photoInputGallery.addEventListener("change", () => handlePhotoChosen(photoInputGallery));
...
document.getElementById("freestyleBtn").addEventListener("click", async () => {
  ...
  try {
    ...
    resultBox.innerHTML = `<p class="hint error">${esc(err.message)}</p>`;
  } finally {
    btn.disabled = false;
...
async function generateHowToCookAi(dishName) {
  ...
  try {
    ...
    document.getElementById("htcRetryBtn").addEventListener("click", () => generateHowToCookAi(dishName));
  }
}
